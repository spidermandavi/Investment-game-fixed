...
function popup(html){
  ...
  let messageDiv = document.getElementById("popupMessage");
  if (!messageDiv) {
    messageDiv = document.createElement("div");
    messageDiv.id = "popupMessage";
    popupContent.insertBefore(messageDiv, popupContent.firstChild);
  }
...
function popupConfirm(message, onConfirm) {
  const popupEl = document.getElementById("popup");
  const popupContent = document.getElementById("popupContent");
  let messageDiv = document.getElementById("popupMessage");
  if (!messageDiv) {
...
function renderInfoBar(){
  ...
  const nameColor=isColorDark(player.color)?"#fff":player.color;
  infoBar.innerHTML=`Turn ${turn} | <span style="color:${nameColor}">${player.name}</span> | Money: $${player.money.toFixed(2)} <button id="infoBtn" style="margin-left:20px;">Info</button>`;
  infoBar.style.background=player.color;
...
function renderStockTable(){
  ...
  stocks.forEach((s,i)=>{
    ...
    if(i===currentPlayer) row.classList.add("currentPlayer");
    row.innerHTML=`
      <td onclick="toggleInfo(${i})" style="cursor:pointer; text-decoration:underline;">${s.name}</td>
      <td>$${s.price.toFixed(2)}</td>
      <td class="${changeClass}">${change.toFixed(2)}</td>
      <td>${s.owned[currentPlayer]}</td>
      <td>${[1,5,10,20,100].map(n=>`<button style="background:${btnColor};color:#fff;" onclick="trade(${i},${n})">${sign}${n}</button>`).join("")}</td>
    `;
