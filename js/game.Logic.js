...
// ===== TURN SYSTEM =====
function endTurn(){
  if(endTurnCooldown) return; // ignore if cooldown active
  endTurnCooldown = true;

  setTimeout(()=> endTurnCooldown = false, 1000); // 1 second cooldown
...
function endGame(force=false){
  if(!force){
    ...
    confirmAction("Are you sure you want to end the game? All progress will be lost.", (confirmed)=>{
      if(confirmed){
        resetGame();
      }
    });
