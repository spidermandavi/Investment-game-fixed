...
function buy(i, amount) {
  saveGameState(); // 🔥 save before change
...
function sell(i, amount = 1) {
  ...
  if (s.owned[currentPlayer] < amount) return popup("Not enough stocks to sell");

  s.owned[currentPlayer] -= amount;
  players[currentPlayer].money += s.price * amount;

  let avg = s.totalSpent[currentPlayer] / (s.owned[currentPlayer] + amount);
  s.totalSpent[currentPlayer] -= avg * amount;
...
function endTurn() {
  ...
  saveGameState(); // 🔥 save BEFORE turn changes
